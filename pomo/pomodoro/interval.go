...
const (
	CategoryPomodoro   Category = "Pomodoro"
	CategoryShortBreak Category = "ShortBreak"
	CategoryLongBreak  Category = "LongBreak"
)
...
func nextCategory(r Repository) (string, error) {
	...
	if err != nil {
		if err == ErrNoIntervals {
			return string(CategoryPomodoro), nil
		} else if err != nil {
			return "", err
		}
	}
	...
	if len(lastBreaks) < 3 {
		return string(CategoryShortBreak), nil
	}
	...
	return string(CategoryLongBreak), nil
}
...
func newInterval(config *IntervalConfig) (Interval, error) {
	...
	switch category {
	case string(CategoryPomodoro):
		i.PlannedDuration = config.PomodoroDuration
	case string(CategoryShortBreak):
		i.PlannedDuration = config.ShortBreakDuration
	case string(CategoryLongBreak):
